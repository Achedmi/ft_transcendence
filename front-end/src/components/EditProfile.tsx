...
function EditProfile(props: EditProfileProps) {
	...
	// 	if (file) {
	// 		// Read the file as binary data and append it to the FormData
	// 		const reader = new FileReader();
	// 		reader.readAsBinaryString(file);
	// 		reader.onload = () => {
	// 			if (reader.result) {
	// 				formData.append("profilePicture", reader.result as string);

	// 				axios
	// 					.patch("l3iba dyalk hh", formData, {
	// 						headers: {
	// 							"Content-Type": "multipart/form-data",
	// 						},
	// 					})
		...
		<motion.div className="flex flex-col mt-1 w-full h-full">
			<div className="flex justify-between items-center px-1 pb-2 border-b-2 border-solid border-dark-cl">
				...
				>
					<Close
						size="38"
						fillColor={!closeHovered ? "#433650" : "#C84D46"}
					/>
				</motion.div>
			</div>
